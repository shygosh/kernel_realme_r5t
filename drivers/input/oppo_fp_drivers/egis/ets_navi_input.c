...
void init_event_enable(struct etspi_data *egistec)
{
...
static void send_key_event(struct etspi_data *egistec, unsigned int code, int value)
{
	struct etspi_data *obj = egistec;
...
{
	struct etspi_data *egistec = (struct etspi_data *)arg;
...
#if TRANSLATED_COMMAND
void translated_command_converter(char cmd, struct etspi_data *egistec)
{
...
void straight_command_converter(char cmd, struct etspi_data *egistec)
{
...
{
	struct etspi_data *egistec = dev_get_drvdata(dev);
...
static DEVICE_ATTR(navigation_enable, 0660,
...
{
	struct etspi_data *egistec = et_spi;
